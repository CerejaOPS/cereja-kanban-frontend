...
### Quando você está começando uma Task do zero:
> "INICIAR TASK: Peguei a F2 (Modal de Perfil e Gamificação). Leia o `docs/CONTEXTO_SISTEMA.md`. Me diga o que eu preciso criar: quais componentes, onde colocar e quais dados mocar inicialmente."

### Quando você travou num erro:
> "Estou tentando criar o ProfileModal mas o Shadcn Dialog não está abrindo. Veja meu código: [cole o código aqui]. O que está errado?"

### Quando você quer entender um conceito:
> "Me explica como funciona a camada `entities` no Feature-Sliced Design. Qual a diferença entre `entities` e `features`?"
